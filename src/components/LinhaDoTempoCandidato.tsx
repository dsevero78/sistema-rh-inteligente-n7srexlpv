...
import { Activity, RotateCw, Plus, Filter } from 'lucide-react'
import {
  CategoriaTimelineCandidato,
  EventoTimelineCandidato,
  candidatosTimelineService,
} from '@/services/candidatosTimeline'
...
interface LinhaDoTempoCandidatoProps {
  candidatoId: string
  vagaTitulo?: string
...
// Configuração visual das 5 categorias conforme especificação:
// CANDIDATURA (azul) — inscrição, origem/canal, triagem
// AVALIAÇÃO (índigo) — matching/score da IA, relatórios gerados, questionário de triagem
// ENTREVISTA (roxo) — agendamento, reagendamento, cancelamento, realização e avaliação pós-entrevista
// GESTÃO (verde) — parecer do gestor contratante, percepção do RH, propostas
// STATUS (laranja) — movimentações de estágio, aprovação, recusa, banco de talentos, onboarding
...
export const LinhaDoTempoCandidato: React.FC<LinhaDoTempoCandidatoProps> = ({
  candidatoId,
  vagaTitulo,
  dataCandidatura,
  onAtualizar,
  ...
  const [eventos, setEventos] = useState<EventoTimelineCandidato[]>([])
  const [carregando, setCarregando] = useState(true)
  const [recarregando, setRecarregando] = useState(false)
  const [filtroCategoria, setFiltroCategoria] = useState<CategoriaTimelineCandidato | 'TODAS'>(
    'TODAS',
  )

  // Modal para adicionar evento manual pelo RH
  const [modalOpen, setModalOpen] = useState(false)
  const [salvando, setSalvando] = useState(false)
  const [formCategoria, setFormCategoria] = useState<CategoriaTimelineCandidato>('GESTÃO')
  const [formTitulo, setFormTitulo] = useState('')
  const [formComplemento, setFormComplemento] = useState('')
  const [formAutor, setFormAutor] = useState(pb.authStore.record?.name || 'Gente & Gestão')
  ...
  const carregarEventos = async (isManualRefresh = false) => {
    ...
    try {
      const lista = await candidatosTimelineService.listarEventos(candidatoId)
      setEventos(lista)
    } catch (err: unknown) {
      console.warn('Erro ao carregar linha do tempo do candidato:', err)
      toast({
  ...
  // Ouvinte Realtime no PocketBase para novos eventos do candidato
  useRealtime<EventoTimelineCandidato>('eventos_timeline_candidato', ({ action, record }) => {
    if (record.candidato === candidatoId) {
      if (action === 'create') {
        setEventos((prev) => {
  ...
  const handleCriarEventoManual = async (e: React.FormEvent) => {
    ...
    setSalvando(true)
    try {
      const novo = await candidatosTimelineService.criarEvento({
        candidato: candidatoId,
        categoria: formCategoria,
        titulo: formTitulo.trim(),
        complemento: formComplemento.trim() || undefined,
        autor: formAutor.trim() || 'Gente & Gestão',
        origem: 'usuario',
      ...
      toast({
        ...
        description: 'O histórico do candidato foi atualizado.',
      })
  ...
  return (
    <Card className="border-slate-200 shadow-xs bg-white overflow-hidden">
      {/* ------------------------------------------------------------------ */}
      {/* HEADER:                                                            */}
      {/* Ícone de pulso (Activity) | "Linha do tempo" | Subtítulo | ● AO VIVO*/}
      {/* ------------------------------------------------------------------ */}
      <div className="px-5 py-4 border-b border-slate-100 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 bg-white">
        <div className="flex items-center gap-2.5">
          ...
          <h3 className="text-lg font-bold text-slate-900 tracking-tight flex items-baseline gap-2 flex-wrap">
            ...
            {(vagaTitulo || dataCandidatura) && (
              <span className="text-sm font-normal text-slate-500">
                {vagaTitulo && <>&middot; {vagaTitulo} </>}
                {dataCandidatura && <>&middot; candidatura {formatarDataCurta(dataCandidatura)}</>}
              </span>
            )}
          </h3>
        </div>
        ...
        <div className="flex items-center gap-3">
          {/* Badge ● AO VIVO pulsante em verde esmeralda monoespaçado */}
      ...
      {/* ------------------------------------------------------------------ */}
      {/* BARRA DE FILTROS POR CATEGORIA (Chips coloridos com contadores)    */}
      {/* ------------------------------------------------------------------ */}
      <div className="px-5 py-2.5 bg-slate-50/60 border-b border-slate-100 flex items-center justify-between gap-3 overflow-x-auto text-xs">
        <div className="flex items-center gap-1.5 flex-nowrap">
          ...
          {(Object.keys(CONFIG_CATEGORIAS) as CategoriaTimelineCandidato[]).map((cat) => {
      ...
      {/* ------------------------------------------------------------------ */}
      {/* CORPO DA TIMELINE: TRILHA VERTICAL VERDE E NÓS                      */}
      {/* ------------------------------------------------------------------ */}
      <CardContent className="p-6 md:p-8 relative">
        {carregando ? (
          ...
          <div className="text-center py-12 px-4 border border-dashed border-slate-200 rounded-xl max-w-lg mx-auto bg-slate-50/50">
            ...
            <p className="text-xs text-slate-400 mt-1 max-w-xs mx-auto">
              {filtroCategoria !== 'TODAS'
                ...
                : 'Ainda não há registros no histórico deste candidato. Clique em "Novo Registro" para adicionar.'}
            </p>
            {filtroCategoria !== 'TODAS' && (
              <Button
                variant="outline"
          ...
          <div className="relative max-w-3xl">
            {/* Trilha vertical verde contínua à esquerda conectando os nós */}
            <div
            ...
            <div className="space-y-6">
              {eventosFiltrados.map((ev, index) => {
                const configCat = CONFIG_CATEGORIAS[ev.categoria] || CONFIG_CATEGORIAS.STATUS
                ...
                return (
                  ...
                  >
                    {/* Nó verde com borda verde e centro verde esmeralda */}
                    ...
                    <div className="flex-1 pt-0.5 space-y-1">
                      ...
                      {/* Título em negrito com complemento em peso normal */}
            ...
            {/* Efeito de fade no rodapé quando há muitos eventos */}
      ...
      {/* ------------------------------------------------------------------ */}
      {/* MODAL PARA RH REGISTRAR EVENTO MANUAL NA TIMELINE                   */}
      {/* ------------------------------------------------------------------ */}
        ...
        <DialogContent className="sm:max-w-md">
          ...
          <form onSubmit={handleCriarEventoManual} className="space-y-4 pt-2">
            <div>
              ...
              <Select
                value={formCategoria}
                onValueChange={(v) => setFormCategoria(v as CategoriaTimelineCandidato)}
              >
                <SelectTrigger className="h-9 text-xs mt-1">
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
                  <SelectItem value="CANDIDATURA">
                    ● CANDIDATURA (Inscrição, Origem, Triagem Inicial)
                  </SelectItem>
                  <SelectItem value="AVALIAÇÃO">
                    ● AVALIAÇÃO (Score IA, Relatórios, Questionários)
                  </SelectItem>
                  <SelectItem value="ENTREVISTA">
                    ● ENTREVISTA (Agendamentos, Realização, Notas RH/Tech)
                  </SelectItem>
                  <SelectItem value="GESTÃO">
                    ● GESTÃO (Parecer do Gestor, Percepção RH, Propostas)
                  </SelectItem>
                  <SelectItem value="STATUS">
                    ● STATUS (Pipeline, Aprovação, Recusa, Banco, Onboarding)
                  </SelectItem>
                </SelectContent>
              </Select>
            </div>
            ...
            <div>
              ...
              <Input
                ...
                placeholder="Ex.: Feedback alinhado com gestor: ou Proposta aceita:"
            ...
            <div>
              ...
              <Textarea
                ...
                placeholder="Ex.: Candidato confirmou disponibilidade para início no dia 15 do próximo mês com salário acordado."
            ...
            <div>
              ...
              <Input
                ...
                placeholder="Ex.: Mariana Siqueira ou seu nome"
